...
class FtSwarm(FtSwarmBase):
    def __init__(self, port: str, serial_handler_class=SerialHandler, debug=False):
        super().__init__()
    ...
    async def send(self, port_name: str, command: str, *args: str | int | float,
                   await_result: bool | None = None) -> int | str | None:
        cmd = self._build_command(port_name, command, args)
        if await_result is None:
            await_result = command != "subscribe"
        result = await self.serial_handler.send_and_wait(cmd, await_result)
    ...
    async def get_compresor(self, port_name: str) -> FtSwarmCompressor:
        return await self._get_object(port_name, FtSwarmCompressor)
