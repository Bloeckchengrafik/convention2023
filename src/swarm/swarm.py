...
class FtSwarmBase:
    ...
    async def send(self, port_name: str, command: str, *args: str | int | float,
                   await_result: bool | None = None) -> int:
        pass
...
class FtSwarmController(FtSwarmIO):
    """
    Use controller commands like `show´ through python
    """

    def __init__(self, swarm: FtSwarmBase, name: str):
        super().__init__(swarm, name)
        self.data: bytes = b""

    async def post_init(self) -> None:
        await self._swarm.send(await self.get_port_name(), "subscribe")

    async def set_value(self, value: str) -> None:
        """
        hex string with spaces without 0x
        """
        self.data = bytes.fromhex(value.replace(" ", ""))

    async def reset_data(self):
        """
        Reset the data to an empty byte string
        """
        self.data = b""

    async def get_data(self) -> bytes:
        """
        Get the data as a byte string
        """
        return self.data

    async def show(self):
        """
        Set the onboard lights white on the target
        """
        await self._swarm.send(await self.get_port_name(), "show")

    async def trigger_user_event(self, *event: int):
        """
        Trigger a user event on the target
        """
        if len(event) > 10:
            raise ValueError("Invalid number of parameters")
        await self._swarm.send(await self.get_port_name(), "triggerUserEvent", *event)

    async def set_microstep_mode(self, mode: MicrostepMode):
        """
        Set the microstep mode for stepper motors

        Only works on the ftPowerDrive
        """
        await self._swarm.send(await self.get_port_name(), "setMicrostepMode", mode.value)

    async def get_microstep_mode(self) -> MicrostepMode:
        """
        Get the microstep mode for stepper motors

        Only works on the ftPowerDrive
        """
        return MicrostepMode(await self._swarm.send(await self.get_port_name(), "getMicrostepMode"))
...
class FtSwarmDigitalInput(FtSwarmInput):
    ...
    async def get_value(self) -> bool:
        return await self._swarm.send(self._port_name, "getValue") == 1
...
class FtSwarmStepper(FtSwarmActor):
    """
    Stepper motor on the ftPowerDrive-I2c-Swarm Bridge
    """
    ...
    async def get_actor_type(self) -> Actor:
        return Actor.STEPPER

    async def set_distance(self, distance: int, relative: bool) -> None:
        """
        Set the distance to drive
        :param distance: distance in steps
        :param relative: True: relative distance, False: absolute position in coordinate system
        """

        await self._swarm.send(self._port_name, "setDistance", distance, relative and 1 or 0)
    ...
    async def get_distance(self) -> int:
        ...
        """
        return await self._swarm.send(self._port_name, "getDistance")

    async def set_position(self, position: int) -> None:
        """
        Set the position in the coordinate system
    ...
    async def get_position(self) -> int:
        """
        Get the position in the coordinate system
        :return: position in steps
        """
        return await self._swarm.send(self._port_name, "getPosition")
    ...
    async def is_running(self) -> bool:
        ...
        """
        return await self._swarm.send(self._port_name, "isRunning") > 0
    ...
    async def set_speed(self, speed: int) -> None:
        ...
        if speed > 4095 or speed < -4095:
            raise ValueError("Speed must be in range -4095..4095")
        await self._swarm.send(self._port_name, "setSpeed", speed)

    async def homing(self, max_distance: int) -> None:
        """
        Start homing
        :param max_distance: maximum distance to drive
        """
        await self._swarm.send(self._port_name, "homing", max_distance)
    ...
    async def is_homing(self) -> bool:
        ...
        """
        return await self._swarm.send(self._port_name, "isHoming") > 0

    async def set_homing_offset(self, offset: int) -> None:
        """
        Set the homing offset
        :param offset: offset in steps
        """
        await self._swarm.send(self._port_name, "setHomingOffset", offset)
